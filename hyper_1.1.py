...
# x = Conv1D(256, num_filter, activation='relu')(x)
# x = Conv1D(256, num_filter, activation='relu')(x)
# x = Conv1D(256, num_filter, activation='relu')(x)
#x = BatchNormalization(epsilon=1e-08, mode=0, axis=1, momentum=0.9, weights=None)(x)
x1 = MaxPooling1D(2)(x1)
...
# x = Conv1D(512, num_filter, activation='relu')(x)
# x = Conv1D(512, num_filter, activation='relu')(x)
# x = Conv1D(512, num_filter, activation='relu')(x)
#x = BatchNormalization(epsilon=1e-08, mode=0, axis=1, momentum=0.9, weights=None)(x)
...
x2 = LSTM(64, return_sequences=True)(embedded_sequences)
...
model.compile(loss='categorical_crossentropy',
              optimizer='adam',
              metrics=['accuracy', f1_score1, f1_score, precision, recall, f2_score])
